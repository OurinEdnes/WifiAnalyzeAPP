#include <iostream>
#include <fstream>
#include <string>
#include <algorithm>
using namespace std;

const int MaxLantai = 10;
const int MaxAP = 10;

string CloneAPSpeed[MaxLantai][MaxAP][2];
int DataBaseUser[MaxLantai];
int TotalLantai = 0;

// Konversi manual dari string ke double
double stringToDoubleManual(const string& s) {
    double result = 0.0, desimal = 0.0;
    bool adaTitik = false;
    double pembagi = 10.0;

    for (char c : s) {
        if (isdigit(c)) {
            if (!adaTitik)
                result = result * 10 + (c - '0');
            else {
                desimal += (c - '0') / pembagi;
                pembagi *= 10;
            }
        } else if ((c == '.' || c == ',') && !adaTitik) {
            adaTitik = true;
        }
    }
    return result + desimal;
}

// Konversi string dengan satuan ke Mbps
double convertToGbpsANDMbps(const string& teks, char typeConvert) {
    double angka = stringToDoubleManual(teks);
    string satuan = teks;
    transform(satuan.begin(), satuan.end(), satuan.begin(), ::tolower);

    typeConvert = toupper(typeConvert);
    if (typeConvert == 'M') {
        if (satuan.find("gbps") != string::npos) return angka * 1000.0;
        if (satuan.find("mbps") != string::npos) return angka;
        if (satuan.find("kbps") != string::npos) return angka / 1000.0;
    } else if (typeConvert == 'G') {
        if (satuan.find("gbps") != string::npos) return angka;
        if (satuan.find("mbps") != string::npos) return angka / 1000.0;
        if (satuan.find("kbps") != string::npos) return angka / 1000000.0;
    }
    return angka;
}

// Ambil data dari file user
void LoadDataAP(const string& username) {
    string path = "C:/xampp/htdocs/WifiAnalyze/BackEnd/UserData(" + username + ")/Wifi-Ap-Speed.txt";
    ifstream file(path);
    string line;
    int lantai = -1, ap = 0;

    while (getline(file, line)) {
        if (line.find(">> Lantai") != string::npos) {
            lantai++;
            ap = 0;
            DataBaseUser[lantai] = 0;
        } else if (line.find("AccessPoint") != string::npos && line.find("||") != string::npos) {
            size_t eq = line.find('=');
            size_t pipe = line.find("||");
            string apSpeed = line.substr(eq + 2, pipe - (eq + 2));

            size_t userStart = line.find("Kecepatan User = ");
            string userSpeed = (userStart != string::npos) ? line.substr(userStart + string("Kecepatan User = ").length()) : "";

            CloneAPSpeed[lantai][ap][0] = apSpeed;
            CloneAPSpeed[lantai][ap][1] = userSpeed;

            ap++;
            DataBaseUser[lantai]++;
        }
    }

    TotalLantai = lantai + 1;
    file.close();
}

// Tukar 2 baris data AP dalam lantai
void swapAP(int lantai, int a, int b) {
    for (int i = 0; i < 2; ++i) {
        string temp = CloneAPSpeed[lantai][a][i];
        CloneAPSpeed[lantai][a][i] = CloneAPSpeed[lantai][b][i];
        CloneAPSpeed[lantai][b][i] = temp;
    }
}

// Sorting dan tampilkan hasilnya
void SortAndShow(char mode) {
    for (int i = 0; i < TotalLantai; ++i) {
        for (int j = 0; j < DataBaseUser[i]; ++j) {
            for (int k = 0; k < DataBaseUser[i] - j - 1; ++k) {
                double diff1 = convertToGbpsANDMbps(CloneAPSpeed[i][k][0], 'M') - convertToGbpsANDMbps(CloneAPSpeed[i][k][1], 'M');
                double diff2 = convertToGbpsANDMbps(CloneAPSpeed[i][k + 1][0], 'M') - convertToGbpsANDMbps(CloneAPSpeed[i][k + 1][1], 'M');
                bool doSwap = (mode == 'A' && diff1 > diff2) || (mode == 'D' && diff1 < diff2);
                if (doSwap) swapAP(i, k, k + 1);
            }
        }
    }

    // Output
    for (int i = 0; i < TotalLantai; ++i) {
        cout << "Lantai " << i + 1 << endl;
        for (int j = 0; j < DataBaseUser[i]; ++j) {
            const string& ap = CloneAPSpeed[i][j][0];
            const string& us = CloneAPSpeed[i][j][1];
            if (ap.empty() || ap == "-" || ap == " ") continue;
            cout << "AccessPoint = " << ap << " || Kecepatan User = " << us << endl;
        }
        cout << "===========================\n";
    }
}

// === MAIN PROGRAM ===
int main(int argc, char* argv[]) {
    if (argc < 3) {
        cout << "ERROR: Argumen tidak lengkap. Gunakan: <username> <A/D>\n";
        return 1;
    }

    string username = argv[1];
    char mode = toupper(argv[2][0]);

    LoadDataAP(username);
    SortAndShow(mode);

    return 0;
}
